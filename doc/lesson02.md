...
- Сделать реализацию слоев приложения для функциональности "еда":
  ...
  - после авторизации запрос попадает в контроллер, методы которого будут доступны снаружи по http, т.е. в запросе может прийти id для еды, не принадлежащее авторизированному пользователю
